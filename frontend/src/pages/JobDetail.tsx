...
function fieldIcon(key: string): { icon: string; purple: boolean } {
  ...
  if (k.includes('revenue') || k.includes('profit') || k.includes('margin') || k.includes('growth')) return { icon: '▲', purple: false }
                ...
                {Object.entries(result.extracted_data).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} className="flex items-start gap-4 px-5 py-3 hover:bg-surface-2 transition-colors">
                      ...
                      <span className="text-xs text-slate-500 w-36 shrink-0 capitalize pt-0.5">{k.replace(/_/g, ' ')}</span>
