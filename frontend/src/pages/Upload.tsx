...
export default function Upload() {
  ...
  const submit = async () => {
    ...
    const interval = setInterval(() => setProgress((p) => (p < 85 ? p + 12 : p)), 120)
    try {
    ...
    <div className="p-8 max-w-2xl">
      ...
      >
        ...
        {file && fileType ? (
          ...
          <>
            <UploadIcon size={28} className="mx-auto mb-3" style={{ color: '#6EE7B7' }} />
        ...
        <div className="mt-4">
          ...
          <div className="h-1.5 bg-surface-3 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-200"
              style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #6EE7B7, #a78bfa)' }} />
          </div>
        </div>
      )}
      ...
      <div className="mt-6 flex gap-3">
        <button className="btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          {uploading ? (
            <><span className="w-3.5 h-3.5 rounded-full border-2 border-surface border-t-transparent animate-spin" />Uploading…</>
          ) : (
            <><CheckCircle size={14} />Submit for processing</>
          )}
        </button>
        <button className="btn-ghost" onClick={() => { setFile(null); setProgress(0) }} disabled={uploading}>Clear</button>
      </div>
    </div>
  )
}